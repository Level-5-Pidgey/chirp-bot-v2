...
export default class CoachRoleAddCommand extends Command {
    public constructor() {
        super("contributor-add",
            {
                aliases : ["add-contributor-role", "insert-contributor-role", "actr", "ictr"],
                args:
                    [
                        {
                            id: "roleToAdd",
                            type: "role",
                            prompt: {
                                start: "Please mention the role you'd like to add to the list of contributor roles.",
                ...
                description : {
                    ...
                    usage : `pointrole insert-contributor-role <role name> <point threshold>`,
                    examples :
                        [
                            `${prefix}pointrole insert-contributor-role @Contributor I 13`,
                            `${prefix}pointrole add-contributor-role @Contributor II 18`,
                        ]
                },
                ratelimit : 3
            });
    ...
    public async exec(message: Message, { roleToAdd, pointThreshold }) : Promise<Message>
    {
        if (message.channel instanceof TextChannel) {
            ...
            {
                await this.client.dbClient.AddContributorRole(roleToAdd, pointThreshold)
                    .then(success =>
                    {
                        if(success)
                        {
